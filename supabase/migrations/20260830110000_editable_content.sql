/*
  ...
  Everything the site says was a TypeScript array literal inside the component
  that rendered it: twenty-six services in services-section.tsx, six FAQs and
  twelve service options in contact-section.tsx, seven differentiators in
  about-section.tsx, five process steps, four statistics, thirty-two design
  images across six categories, two before/after pairs, and every eyebrow,
  heading and line of hero copy. Changing a phone number, a price bracket or a
  photograph meant editing a component, rebuilding and redeploying.

  These tables move all of it into the database, so it is edited in the
  Supabase table editor and picked up by the site on its next revalidation.
  ...
  The two exceptions are `site_settings`, keyed by its setting name, and
  `section_headings`, keyed by its section — both are maps rather than lists,
  so neither needs an ordering or a published flag.
  ...
  browser, and a policy allowing writes would let anyone rewrite the site's
  copy.

  The application never breaks on missing data — lib/content/fetch.ts falls
  back to lib/content/defaults.ts per table — so this migration is safe to
...
/*
  Applies the columns, RLS, policy, index and updated_at trigger that every
  list table shares.

  A procedural loop rather than eleven copies of the same twenty lines: the
  copies would drift, and the one that drifted would be the table whose RLS was
  never enabled. public.set_updated_at() is created by
...
BEGIN
  FOREACH target_table IN ARRAY ARRAY[
    'service_groups', 'services', 'about_features', 'process_steps',
    'stats', 'faqs', 'design_categories', 'design_images',
    'before_after', 'service_options'
  ]
  LOOP
    EXECUTE format(
...
-- nowhere, which is a confusing thing to debug from the table editor. These
...
-- Seed
--
-- Generated from lib/content/defaults.ts by scripts/generate-content-seed.mjs.
-- Do not edit the block below by hand: change the defaults and regenerate, so
-- the rows and the fallback the site uses when the database is unreachable
-- stay the same data.
-- ---------------------------------------------------------------------------
...
INSERT INTO public.site_settings (key, value) VALUES
  ...
  ('contact.maps_href', 'https://www.google.com/maps/search/?api=1&query=New+Cairo%2C+Egypt'),
  ...
  ('hero.subheadline', 'نصمم، ننفذ، ونشرف على جميع أعمال التشطيبات والمقاولات بأعلى معايير الجودة والاحترافية'),
  ...
  ('about.statement_body', 'خبرة تمتد لأكثر من 15 عاماً في تنفيذ مشاريع سكنية وتجارية فاخرة بأعلى معايير الجودة العالمية، من التصميم الأولي وحتى تسليم المفتاح.'),
  ('about.image_url', 'https://images.pexels.com/photos/7722168/pexels-photo-7722168.jpeg?auto=compress&cs=tinysrgb&w=1920'),
  ('hero.image_url', 'https://images.pexels.com/photos/33529500/pexels-photo-33529500.jpeg?auto=compress&cs=tinysrgb&w=2560'),
  ('stats.image_url', 'https://images.pexels.com/photos/35300835/pexels-photo-35300835.jpeg?auto=compress&cs=tinysrgb&w=1920'),
...
INSERT INTO public.services (id, group_slug, icon, title, description, sort_order) VALUES
  ('finishing-apartments', 'finishing', 'Home', 'تشطيب الشقق', 'تصاميم عصرية وجودة عالية', 10),
  ('finishing-villas', 'finishing', 'Building2', 'تشطيب الفلل', 'فلل فاخرة بأدق التفاصيل', 20),
  ('finishing-offices', 'finishing', 'Briefcase', 'تشطيب المكاتب', 'مساحات عمل احترافية', 30),
  ('finishing-shops', 'finishing', 'Store', 'تشطيب المحلات', 'تصاميم تجارية جذابة', 40),
  ('finishing-clinics', 'finishing', 'Stethoscope', 'تشطيب العيادات', 'بيئات طبية نزيهة ومريحة', 50),
  ('finishing-restaurants', 'finishing', 'UtensilsCrossed', 'المطاعم والكافيهات', 'أجواء استثنائية لا تُنسى', 60),
  ('finishing-companies', 'finishing', 'Building', 'تشطيب الشركات', 'مقرات تعكس الاحترافية', 70),
  ('design-interior', 'design', 'Sofa', 'تصميم داخلي', 'تصاميم فاخرة تناسب ذوقك', 10),
  ('design-exterior', 'design', 'Palette', 'تصميم خارجي', 'واجهات معمارية لافتة', 20),
  ('design-2d', 'design', 'Ruler', 'تصميم 2D', 'مخططات دقيقة وشاملة', 30),
  ('design-3d', 'design', 'Box', 'تصميم 3D', 'مشاهدة واقعية قبل التنفيذ', 40),
  ('design-gardens', 'design', 'Trees', 'تصميم حدائق', 'مساحات خضراء ساحرة', 50),
  ('design-landscape', 'design', 'Flower2', 'تصميم المناظر', 'تنسيق خارجي متكامل', 60),
  ('design-entrances', 'design', 'DoorOpen', 'المداخل', 'انطباع أول قوي', 70),
  ('design-facades', 'design', 'Sun', 'الواجهات', 'واجهات مبتكرة وعصرية', 80),
  ('specialized-lighting', 'specialized', 'Zap', 'الإضاءة', 'أنظمة تخلق الأجواء المثالية', 10),
  ('specialized-plumbing', 'specialized', 'Droplets', 'السباكة', 'أنظمة صحية متكاملة', 20),
  ('specialized-gypsum', 'specialized', 'Layers', 'الجبس بورد', 'تشكيلات ديكورية أنيقة', 30),
  ('specialized-paint', 'specialized', 'Paintbrush', 'الدهانات', 'دهانات فاخرة ودائمة', 40),
  ('specialized-flooring', 'specialized', 'Grid3x3', 'الأرضيات', 'أفضل الخامات والتشطيبات', 50),
  ('specialized-marble', 'specialized', 'Gem', 'الرخام', 'أعمال رخام فاخرة', 60),
  ('specialized-carpentry', 'specialized', 'TreePine', 'النجارة', 'دقة وخامات ممتازة', 70),
  ('specialized-aluminium', 'specialized', 'DoorOpen', 'الألمنيوم', 'ألمنيوم حراري وديكوري', 80),
  ('specialized-smart-home', 'specialized', 'Cpu', 'السمارت هوم', 'أنظمة منزل ذكي متكاملة', 90),
  ('specialized-restoration', 'specialized', 'RefreshCw', 'الترميم', 'تجديد بلمسة عصرية', 100),
  ('specialized-maintenance', 'specialized', 'Wrench', 'الصيانة', 'صيانة دورية احترافية', 110)
ON CONFLICT (id) DO UPDATE SET
...
INSERT INTO public.about_features (id, icon, title, description, sort_order) VALUES
  ('quality', 'Award', 'جودة فائقة', 'أعلى معايير الجودة في كل تفصيلة', 10),
  ('team', 'Users', 'فريق محترف', 'نخبة من المهندسين والفنيين', 20),
  ('punctuality', 'Clock', 'التزام بالمواعيد', 'تسليم في الوقت المحدد دون تأخير', 30),
  ('modern-design', 'Sparkles', 'أحدث التصاميم', 'نواكب أحدث الاتجاهات العالمية', 40),
  ('pricing', 'DollarSign', 'أسعار تنافسية', 'أفضل قيمة دون تنازل عن الجودة', 50),
  ('warranty', 'ShieldCheck', 'ضمان شامل', 'ضمان على جميع الأعمال', 60),
  ('aftercare', 'Headset', 'ما بعد التسليم', 'صيانة ومتابعة بعد التسليم', 70)
ON CONFLICT (id) DO UPDATE SET
    icon = EXCLUDED.icon,
    title = EXCLUDED.title,
    description = EXCLUDED.description,
...
INSERT INTO public.process_steps (id, icon, title, description, sort_order) VALUES
  ('consultation', 'MessageSquare', 'الاستشارة والمعاينة', 'نستمع لرؤيتك ونزور الموقع لتحديد المتطلبات', 10),
  ('design-2d', 'PencilRuler', 'التصميم 2D', 'مخططات أولية دقيقة للمساحة', 20),
  ('design-3d', 'Box', 'التصور 3D', 'ترى مشروعك واقعياً قبل التنفيذ', 30),
  ('quote', 'FileText', 'العرض والتعاقد', 'عرض سعر مفصل وشفاف بلا رسوم خفية', 40),
  ('delivery', 'KeyRound', 'التنفيذ والتسليم', 'تنفيذ بأعلى المعايير حتى تسليم المفتاح', 50)
ON CONFLICT (id) DO UPDATE SET
    icon = EXCLUDED.icon,
    title = EXCLUDED.title,
    description = EXCLUDED.description,
...
INSERT INTO public.stats (id, emoji, target, suffix, label, sort_order) VALUES
  ('projects', '🏗️', 450, '+', 'مشروع منجز', 10),
  ('experience', '⭐', 15, '+', 'سنة خبرة', 20),
  ('clients', '😊', 380, '+', 'عميل سعيد', 30),
  ('area', '📐', 250000, ' م²', 'مساحة منجزة', 40)
ON CONFLICT (id) DO UPDATE SET
...
INSERT INTO public.faqs (id, question, answer, sort_order) VALUES
  ('duration', 'ما هي مدة تنفيذ المشروع؟', 'تختلف مدة التنفيذ حسب نوع وحجم المشروع. الشقق السكنية تستغرق عادة 60-90 يوماً، بينما الفلل قد تستغرق 120-180 يوماً. نقدم لك جدولاً زمنياً دقيقاً بعد الاستشارة الأولى.', 10),
  ('warranty', 'هل تقدمون ضماناً على الأعمال؟', 'نعم، نقدم ضماناً شاملاً على جميع أعمالنا. مدة الضمان تختلف حسب نوع العمل، وتصل إلى سنتين للأعمال الإنشائية وسنة للتشطيبات والديكورات.', 20),
  ('preview', 'هل يمكنني رؤية المشروع قبل التنفيذ؟', 'بالتأكيد. نوفر تصاميم ثلاثية الأبعاد وعروضاً واقعية لمشروعك قبل بدء التنفيذ، حتى تتمكن من رؤية كل تفصيلة والموافقة عليها.', 30),
  ('pricing', 'كيف يتم تحديد تكلفة المشروع؟', 'نقوم بزيارة الموقع مجاناً ثم نقدم عرض سعر مفصلاً وشفافاً يشمل جميع التكاليف بدون أي رسوم خفية. السعر يعتمد على المساحة، الخامات المطلوبة، ونوع التشطيب.', 40),
  ('coverage', 'هل تعملون في جميع المحافظات؟', 'نعمل في جميع المحافظات الرئيسية بجمهورية مصر العربية. للاستفسار عن توفر الخدمة في منطقتك، يرجى التواصل معنا عبر نموذج الاتصال أو الواتساب.', 50),
  ('payment', 'ما هي طرق الدفع المتاحة؟', 'نقدم خطط دفع مرنة على دفعات مرتبطة بمراحل المشروع. نقبل التحويل البنكي والشيكات. يتم الاتفاق على جدول الدفع في عقد المشروع.', 60)
ON CONFLICT (id) DO UPDATE SET
...
INSERT INTO public.before_after (id, title, before_image, after_image, sort_order) VALUES
  ('elite-apartment', 'شقة النخبة - التجمع الخامس', 'https://images.pexels.com/photos/15087186/pexels-photo-15087186.jpeg?auto=compress&cs=tinysrgb&w=1920', 'https://images.pexels.com/photos/7546323/pexels-photo-7546323.jpeg?auto=compress&cs=tinysrgb&w=1920', 10),
  ('jasmine-villa', 'فيلا الياسمين - الشيخ زايد', 'https://images.pexels.com/photos/19408681/pexels-photo-19408681.jpeg?auto=compress&cs=tinysrgb&w=1920', 'https://images.pexels.com/photos/16573669/pexels-photo-16573669.jpeg?auto=compress&cs=tinysrgb&w=1920', 20)
ON CONFLICT (id) DO UPDATE SET
    title = EXCLUDED.title,
...
INSERT INTO public.service_options (id, label, sort_order) VALUES
  ('apartment', 'تشطيب شقة', 10),
  ('villa', 'تشطيب فيلا', 20),
  ('office', 'تشطيب مكتب', 30),
  ('clinic', 'تشطيب عيادة', 40),
  ('restaurant', 'تشطيب مطعم', 50),
  ('shop', 'تشطيب محل تجاري', 60),
  ('interior-design', 'تصميم داخلي', 70),
  ('exterior-design', 'تصميم خارجي وواجهات', 80),
  ('landscaping', 'تنسيق حدائق', 90),
  ('restoration', 'ترميم وتجديد', 100),
  ('supervision', 'إشراف هندسي', 110),
  ('other', 'أخرى', 120)
ON CONFLICT (id) DO UPDATE SET
    label = EXCLUDED.label,
    sort_order = EXCLUDED.sort_order;
