 ...
 *
 * The precedence is deliberate and worth stating, because there are now three
 * places a phone number could come from:
 *
 *   1. `site_settings` in Supabase — wins. It is the one an editor can change
 *      without a developer, and the whole point of this layer.
 ...
 * old one would be worse than not being editable at all, so an overridden
 * number is put back through `buildContactLinks` and all five forms move
 * together.
 */
...
export function resolveSiteDetails(settings: SiteSettings) {
  ...
  return {
    ...
    hours: {
      days,
      time,
      // Derived, not stored. The WhatsApp tooltip wants the two on one line;
      // storing that as a third setting would let it disagree with the pair it
      // summarises the moment someone edits only the days.
      summary: `${days}: ${time}`,
    },
    warranty: {
 ...
 * stray space has to fall back rather than reach that function as NaN and
 * render "NaN سنة" on the live site.
 */
